...
public class ServerLoginInteractor implements ServerLoginInputBoundary {
    ...
    /**
     * Handles a login packet received from a com.ikun.client, processing the login request and responding accordingly.
     *
    ...
    private void handlePacket(PacketIn<PacketClientLogin> packetIn) {
        ...
        if (info.getStatus() == ConnectionInfo.Status.LOGGED) {
            serverLoginPresenter.addMessage("Login Failed: com.ikun.client " + info.getUser().getUsername() + "already logged in");
